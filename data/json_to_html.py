...
def convert_json_to_html(team_name):
    ...
    if team_name in failed_teams:
        with open(html_file, "w", encoding="utf-8") as f:
            f.write(f"""
            <html>
            <head><title>{team_translation.get(team_name, team_name)} 경기 데이터</title></head>
            <body>
                <h2 style="text-align: center;">{team_translation.get(team_name, team_name)} 경기 데이터</h2>
                <p style="color: red; text-align: center;">⚠️ 데이터 수집 실패! 최신 경기 정보를 가져올 수 없습니다.</p>
            </body>
            </html>
            """)
